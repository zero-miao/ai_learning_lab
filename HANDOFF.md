...
- 掌握度评估支持显式保存作答草稿；出题和阅卷任务可在刷新后恢复进度，失败时在页面内展示原因并直接重试。已评分结果可查看逐题答案、得分和反馈，完成一次评估后可继续生成新评估。
- 复习计划 `/reviews` 支持查看记录、生成复习提示、提交复盘和跳转 Topic；提示生成与复盘评分任务按复习记录独立轮询，刷新后可恢复进度，失败时支持页面内重试。复习提示使用 Markdown/GFM 渲染。
...
- 评估与复习任务反馈增强后，前端 `npm run build`、`npm run lint` 及后端 26 项完整回归再次通过；`manage.py check`、`makemigrations --check --dry-run` 和 `ruff check backend` 均通过。
...
1. 跨 Topic 标注查看：产品设计要求阅读页可在“当前 Topic 标注 / 全部标注”之间切换，当前仅传入当前 Topic 的标注。
2. 视频进度条标记：当前已在播放器下方展示可点击的学习标记列表，但尚未将标记点绘制到播放器进度条。
3. 关键 V2 用户路径的端到端自动化测试尚未建立。

## 10. 接手优先级

1. 补齐阅读页跨 Topic 标注切换，并完成外挂字幕、思维导图和评估/复习任务反馈的浏览器验收。
2. 为关键 V2 用户路径补端到端测试，至少覆盖视频导入 -> ASR -> 清洗 -> 字幕 seek -> 划词 Locator。
3. 将视频学习标记接入播放器进度条。
4. 之后再处理前端代码分割和 bundle 体积告警（当前主 bundle 约 1.79 MB，gzip 约 560 kB）。
