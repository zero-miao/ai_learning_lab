...
- Topic 统一为学习话题，不再区分学习型和讨论型；支持搜索和删除。Topic 关联的业务实体按模型外键规则清理，但讨论 Session 与通用 AITask 尚未随 Topic 删除。
...
- 问答系统升级：
    - **Session 会话界面**：后端已补全 `SessionViewSet` 及消息路由，前端可展示会话消息并提交追问；当前 `AnswerQuestionTask` 尚未消费追问内容与历史消息，因此仍未形成真正的多轮问答。
    - **历史展示**：支持从问答列表进入已有 Session 查看消息；历史上下文继承仍待后端补齐。
...
- **问答引用定位**：Locator 已正确保存并展示选中文字；`AnswerQuestionTask` 仍存在 `source_text` / `selected_text` 键名不一致，AI 提示词引用尚未修复。
...
1. 阅读问答真正的多轮对话：`AnswerQuestionTask` 需要使用本次追问、Session 历史和 Locator 选中文字组装上下文；当前任务仍重复回答初始 `question_text`，且 `source_text` / `selected_text` 键名不一致。
2. Topic 删除闭环：删除 Topic 时需要同步清理讨论 Session、消息及相关通用 AITask，避免孤立记录。
3. 评估草稿持久化：后端已有 `POST /api/exams/{id}/save/`，前端尚未调用。
4. 复习任务进度：生成提示和提交复盘后，前端尚未轮询和展示 AITask 的执行、失败及重试状态。
5. 关键 V2 用户路径的端到端自动化测试尚未建立。

## 10. 接手优先级

1. 修复阅读问答多轮上下文、选中文字传递，并补齐 Topic 删除时的 Session/AITask 清理。
2. 接通评估草稿前端保存和复习任务进度、失败、重试反馈。
3. 完成外挂字幕、思维导图和历史评估结果的浏览器验收。
