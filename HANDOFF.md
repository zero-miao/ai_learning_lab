...
- 数据库中真实使用反馈 #1-#22 已全部完成。最新一轮将“查找材料”收进学习讨论输入区作为快捷指令，补齐任务进度与完成反馈；阅读问答可用本地 SearxNG 检索互联网背景资料并标注来源，检索不可用时仍按材料回答；补料默认排除维基百科、微信读书和豆瓣域名。
...
- 后端 `manage.py test api` 共 44 项通过，覆盖系统配置、当前阅读偏好独立更新、Provider 模型发现、管理助手查询、批量变更、幂等与并发保护、带互联网背景资料的多轮问答、Topic 置顶与删除、集合摘要、统一分页与查询索引、补料人工采纳、相关度下限与排除域名、TTS、跨 Topic 标注、反馈记录和视频学习全链路等关键契约。
