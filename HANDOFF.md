...
- 阅读知识产出：学习助手收敛为单一入口，使用问答、概念和高亮 Tab；正文以不同样式标记
  已提问片段、概念来源和高亮。阅读前导支持基础 Markdown 渲染，并默认折叠。
- 考试自动保存：待作答考试在输入停止后自动保存，并在无变化时每 10 秒保存一次。
  学习产出可直接查看已沉淀问答；主图连线和关系标签均可直接点击编辑。
- 知识产出交互：问答任务会向模型显式传递选中原文；正文中的概念、问答与高亮均可点击，
  并在学习助手中打开对应条目。概念支持编辑、确认和删除；主图支持拖拽节点到另一节点以建立关系，
  并可删除概念及其关联关系。
- 阅读前导：支持基础 Markdown 表格，默认展开并采用信息卡片样式突出显示。
- 考试可用性：支持 Ctrl/Cmd+S 手动保存，并明确显示草稿保存中、已保存、未保存和失败状态。
- 标记与主图：概念用文字颜色区分草稿与已确认状态，高亮保留背景色，问答以小问号提示；
  问答支持删除和精确回跳。拖拽重复关联会打开已有关系编辑，主图不再提供独立的建立关系按钮。
...
10. 阅读前导支持折叠和基础 Markdown；阅读知识产出以概念、问答和高亮三种样式标记。
    阅读器侧栏使用单一入口。考试支持输入后自动保存及 10 秒定时保存；学习产出可直览已沉淀问答，
    思维导图连线支持点击编辑。
11. 问答任务会包含用户选中的原文；阅读器中点击任意概念、问答或高亮区间会打开右侧对应条目。
    概念草稿与已确认概念分别使用不同正文样式，并支持编辑、确认、删除。阅读前导支持 Markdown
    表格且默认展开。主图支持拖拽节点建立关系，概念详情中可删除概念及关联关系。
12. 考试草稿支持 Ctrl/Cmd+S，且 UI 显示保存状态。阅读标记允许概念、问答和高亮重叠：
    概念以文字颜色、问答以小问号、高亮以背景色表示。问答可删除并精确回跳。拖拽已有关联的
    两个节点会打开该关系编辑，而不会创建重复关系。
