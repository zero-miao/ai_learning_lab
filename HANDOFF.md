...
- `UniversalReader` 渲染 HTML 清洗文本；支持系统默认深浅色、手动切换、阅读前导和文本选区菜单。
...
- 学习助手只有一个浮动入口，包含问答、问答历史、概念和高亮 Tab；点击 Drawer 外区域会关闭它。列表条目的操作位于标题行，以图标呈现，正文可完整显示。

### 学习产出与思维导图

- 学习型 Topic 的“学习产出”使用问答、概念、高亮 Tab，标题显示各自数量；每项都可以跳回原文。
